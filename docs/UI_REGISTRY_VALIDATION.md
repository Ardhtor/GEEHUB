...
That duplication is useful at the surface but creates a drift risk. A project can remain in the machine-readable registry while its name, status, group, summary, or repository link becomes stale in the browser doorway.

`hub/validate-ui-registry.py` is a dependency-free provenance guard. It checks:

1. every registry entry has matching `name`, `group`, `status`, and `summary` data in the embedded browser tuple;
