#!/usr/bin/env python3
"""Check that the browser registry exposes the declared repository links.
...
browser to run; it only prevents the two files from silently drifting on the
small set of fields that affect cross-repository navigation.
"""
...
def main() -> int:
    ...
    for project in projects:
        ...
        if project.get("path") != expected_url:
            failures.append(
                f"{project.get('id', '<unknown>')}: registry path must equal {expected_url}"
            )
        if expected_url not in html:
            failures.append(
                f"{project.get('id', '<unknown>')}: {expected_url} is missing from index.html"
            )
    ...
    print(f"UI registry valid: {sum(1 for p in projects if p.get('repo'))} repository links checked.")
    return 0
